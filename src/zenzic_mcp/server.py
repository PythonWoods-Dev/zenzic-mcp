...
async def _handle_call_tool(
    ...
    try:
        ...
        # Any other failure (an adapter crash, a config error, a rule-engine
        # bug) must surface as a clean tool result, not propagate past the
        # MCP wire boundary -- confirmed live that an uncaught exception here
        # doesn't kill the server process, but does leak as a raw MCPError on
        # the client side instead of a controlled is_error result.
        return types.CallToolResult(
